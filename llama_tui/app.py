...
# VERIFICATION_STATUSES moved to model_loader.py and re-imported above.
...
# dataclass_payload moved to models.py so model_loader.py can import it
# without an app.py → model_loader cycle. Re-imported below.
...
class AppConfig:
    ...
    # _load_model body moved to model_loader.load_model_from_payload; this
    # wrapper keeps the method shape so test patches and subclass overrides
    # continue to work.
    def _load_model(self, raw: object, index: int) -> ModelConfig:
        return load_model_from_payload(raw, index)
