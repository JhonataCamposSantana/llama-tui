...
from .moe_tuning import (
    _best_mtp_acceptance_draft_for_model,
...
BENCHMARK_SAMPLE_TIMEOUT = _env_int_override(
    'LLAMA_TUI_BENCHMARK_SAMPLE_TIMEOUT', 240, minimum=10,
)
BENCHMARK_READY_TIMEOUT = _env_int_override(
...
# ADAPTIVE_CONTEXT_ROUNDING, ADAPTIVE_BINARY_STEPS, ADAPTIVE_MAX_CONTEXT_PROBES
# now live in adaptive_search.py and are re-imported below for backwards
# compatibility with the many call sites that read them via benchmark.* .
ADAPTIVE_MAX_MEASUREMENTS = _env_int_override(
    'LLAMA_TUI_ADAPTIVE_MAX_MEASUREMENTS', 20, minimum=1,
)
# Context-ladder constants moved to adaptive_search.py; re-imported below.
...
SMART_BENCHMARK_SOFT_BUDGET_SECONDS = _env_int_override(
    'LLAMA_TUI_SMART_BUDGET_SECONDS', 45 * 60, minimum=60,
)
FULL_RUNTIME_PROFILE_BUDGET_SECONDS = _env_int_override(
    'LLAMA_TUI_FULL_BUDGET_SECONDS', 120 * 60, minimum=60,
)
...
# SMART_MAX_FULL_CONTEXTS_PER_VARIANT moved to adaptive_search.py (re-imported).
...
# score_fast_chat / score_long_context / score_opencode_ready /
# score_auto moved to scoring.py and re-imported below.
...
# SUITE_BENCHMARK_STATE_FIELDS moved to full_suite.py.
...
# _suite_restore_config_fields moved to full_suite.py.
...
# _latest_benchmark_run moved to full_suite.py.
...
# _mtp_acceptance_stage_status moved to full_suite.py.
...
# full_suite_summary_text moved to full_suite.py.
...
def benchmark_completion_suite(
    ...
    samples = []
    failures = []
    active_prompts = tuple(prompts or BENCHMARK_PROMPTS)
    if not active_prompts:
        return False, {'error': 'no benchmark prompts configured'}
...
# _model_mtp_acceptance_records / _usable_mtp_acceptance_record_for_model /
# _best_mtp_acceptance_draft_for_model / _moe_tuning_mtp_aware /
# _moe_tuning_mtp_required / _moe_tuning_mtp_blocked_reason /
# _moe_tuning_mtp_acceptance_required_reason moved to moe_tuning.py.
...
# _model_has_nextn_or_recurrent_features and _mtp_aware_moe_tuning_profile
# moved to moe_tuning.py.
...
# moe_context_bucket_specs and _context_validation_n_cpu_moe_values moved
# to moe_tuning.py.
